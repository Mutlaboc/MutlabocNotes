...
Основная рабочая ветка Android-приложения - `develop`. Backend-репозиторий
`Mutlaboc/notes-backend` использует такой же рабочий процесс: ежедневная
интеграция идет через `develop`, а `master` остается стабильной release-веткой.
